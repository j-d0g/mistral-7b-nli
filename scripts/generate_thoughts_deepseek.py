...
from llm.deepseek_api import DeepSeekAPI # Changed Import
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("deepseek_generation.log"), # Changed log file name
...
# --- Argument Parsing ---
parser = argparse.ArgumentParser(description='Generate Chain-of-Thought augmentations for NLI examples using DeepSeek API.')
...
parser.add_argument('--failed-csv', type=str, default='failed_deepseek_generation.csv', help='Path to save details of failed examples.') # Changed default
parser.add_argument('--model-name', type=str, default='deepseek-chat', help='Name of the DeepSeek model to use (e.g., deepseek-chat, deepseek-reasoner).') # Changed default
...
def process_chunk(chunk_df, api_key, model_name, json_schema, output_json, failed_csv_path, failed_lock, rate_limit_per_worker, worker_id, results_dict):
    ...
    for index, row in chunk_df.iterrows():
        with rate_limiter:
            ...
            # Use DeepSeekAPI client
            llm = DeepSeekAPI(api_key=api_key) 
            try:
                # Pass necessary arguments for predict_label, which internally calls llm.generate_text
                response_json = predict_label(
                    ...
                    json_filepath=output_json # Used by persist_benchmarks inside predict_label
                )
...
def main():
    ...
    load_dotenv()
    api_key = os.getenv('DEEPSEEK_API_KEY') # Changed environment variable name
    if not api_key:
        ...
        for index_str in sorted_indices:
            ...
            if 'response' in results_dict[index_str]:
                ...
                outfile.write('\n')
                 ...
                 logger.error(f"Missing 'response' key for index {index_str} in results_dict") # Log missing response
    ...
    with open(summary_file_path, "w") as summary_file:
        summary_file.write(f"Results Summary (DeepSeek Parallel - {num_workers} workers, Model: {args.model_name}):\n") # Updated summary title
