...
*   **Rank (`r`) and Alpha (`lora_alpha`):**
    ...
    *   **Experimentation Findings & Final Rationale:**
        *   Initial runs with conservative settings (`r=16`, `lora_alpha=32`, effective batch size 16) proved effective for rapid prototyping and achieved good initial convergence.
        *   However, the inclusion of *reflected* thought processes (generated by `open-mistral-nemo` to correct `open-mistral-7b`'s errors) introduced significant complexity and diversity into the training data. It contained potentially conflicting reasoning styles compared to the initially correct examples.
        *   Based on the hypothesis that this complex data required more model capacity and longer training (supported by observing continued loss decrease past epoch 1 even with simpler models), the configuration was revisited.
        *   The final configuration adopted **`r=32`, `lora_alpha=64`**. This higher rank was chosen to provide the necessary capacity to model the diverse reasoning paths.
        *   Crucially, this was combined with **stability measures**: an increased effective batch size (e.g., 64) to average over noisy/diverse examples, gradient clipping, and a lower learning rate. This stabilized training for the more complex dataset and higher-rank model.
        *   Ongoing experiments indicate this combination (`r=32/a=64` + larger batch + stability measures) yields the best performance, validating the hypothesis.
...
*   **Other:**
    ...
    *   `gradient_checkpointing=True`: Crucially for memory savings.
...
*   **Logged Metrics:** The `SFTTrainer` automatically logs training/evaluation loss, learning rate, throughput (samples/sec), and GPU memory usage. If enabled, it also logs token accuracy.
*   **Custom Metrics:** To log NLI-specific classification metrics like accuracy, precision, recall, F1 during the validation phase of training, a `compute_metrics` function needs to be implemented in `train_sft.py` and passed to the `SFTTrainer`. This function typically takes an `EvalPrediction` object, extracts predictions and labels, calculates metrics, and returns a dictionary. (**Note:** As of the current implementation, this is not included; validation primarily relies on monitoring `eval_loss` and token accuracy. Final classification metrics are obtained post-training via `evaluate/run_inference.sh`).
...
*   **Library Compatibility Issues:**
    ...
    *   **Solutions:**
        ...
        *   Be aware of breaking changes in library APIs (e.g., `